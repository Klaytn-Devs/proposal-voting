...
const Home = () => {
  ...
  return (
    <>
      ...
      {!isLoadingFetchList && (
        <div className="text-md z-10 text-center text-red-700">
          <div className="font-bold text-lg">VOTING DURATION</div>
          <div>{formatTime(dataTime[0]) + ' UTC ~ ' + formatTime(dataTime[1]) + ' UTC'}</div>
        </div>
      )}
      {!isLoadingFetchList && data && data.length > 0 && (
        <div className="z-10 mt-5 font-bold">
          TOTAL DEPOSITED: {dataTotalVote} PGEV
        </div>
      )}
      <div className="my-5 grid w-full max-w-screen-xl grid-cols-1 gap-5 px-5 md:grid-cols-2 xl:px-0">
        {data &&
          ...
          data
            .map((e) => ({
              title: e.name,
              description: e.description?.substring(0, 512),
              amountVote: e.amountVote,
              demo: (
                <div className="group relative flex flex-1 cursor-pointer items-center justify-center">
                  ...
                  <div className="grid grid-cols-1 items-center gap-1">
                    ...
                    <div className="text-center">
                      <button
                        ...
                        className="inline-flex items-center rounded-md border border-red-700 bg-red-700 p-1.5 px-4 text-center text-sm text-white transition-all hover:bg-white hover:text-red-700"
                        onClick={(e) => {
                          e.preventDefault()
                          onClickVote(index + 1)
                        }}
